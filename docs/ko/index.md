...
4. **실전 가이드 활용**: 일상적인 파일 관리 및 자주 묻는 질문은 [제10장: 실전 문제 해결 및 레시피](faq_howtos.md)에서 확인하세요.
...
1. **듀얼 매트릭스 단축키 마스터**: macOS 기본 단축키와 클래식 커맨더 기능 키를 비교한 전체 매트릭스를 [제9장: 마스터 키보드 단축키 레퍼런스](keyboard_shortcuts.md)에서 확인하세요.
...
4. **시스템 모니터링 및 스토리지 유지 관리**: [제7장: 시스템 도구 및 유지 관리](system_tools.md)에서 실시간 리소스 모니터 HUD (`⌘⇧M`), 시각적 트리맵 디스크 분석기 (`⌘⇧D`), 안전한 시스템 클리너 (`⌘⇧C`), 심층 앱 제거기 (`⌘⇧U`)를 사용하세요.
5. **사용자 환경 설정 및 이식**: [제8장: 환경설정 및 사용자 정의](preferences_and_customization.md)에서 명령 재지정, 자동 새로고침 설정 및 구성 XML을 내보냅니다.
...
복잡한 파일 관리 문제 자동화: 토큰 및 RegEx 대체를 사용한 일괄 다중 이름 바꾸기(`Ctrl+M`), 병렬 시각적 파일 차이(`Meta+Shift+F12`), 양방향 디렉터리 동기화 (Sync Dirs) (Sync Dirs) (Sync Dirs)(`Shift+F12`), "목록 상자에 피드"를 사용한 고급 다중 필터 검색(`Alt+F7`), 스포트라이트 및 자연어 의미 체계 명령(`/`), 파일 분할기 및 링커, 체크섬 확인(MD5, SHA-256, CRC32), 보안 멀티패스 삭제(`cm_Wipe`) 및 내장 터미널(`Ctrl+J`).
...
### 🛠️ [제7장: 시스템 도구 및 유지 관리](system_tools.md)
툴바 상태 캡슐 HUD 또는 전체 진단 콘솔 (`cm_SystemStatus`)에서 실시간 시스템 성능을 모니터링하고, 인터랙티브 트리맵 (`cm_DiskUsageAnalyzer`)으로 디스크 사용량을 분석하며, 2단계 위험 분석 (`cm_CleanSystem`)으로 캐시, 로그 및 개발 아티팩트를 안전하게 정리하고, `~/Library/`의 분산된 고아 잔여물을 포함한 완벽한 앱 삭제 (`cm_UninstallApp`)를 수행합니다.

### ⚙️ [제8장: 환경설정 및 사용자 정의](preferences_and_customization.md)
...
### ⌨️ [제9장: 마스터 키보드 단축키 레퍼런스](keyboard_shortcuts.md)
...
### ❓ [제10장: 실전 문제 해결 및 레시피](faq_howtos.md)
...
### 📥 [제11장: 다운로드 및 설치 가이드](download.md)
