...
복잡한 파일 관리 문제 자동화: 토큰 및 RegEx 대체를 사용한 일괄 다중 이름 바꾸기(`Ctrl+M`), 병렬 시각적 파일 차이(`Meta+Shift+F12`), 양방향 디렉터리 동기화 (Sync Dirs)(`Shift+F12`), "목록 상자에 피드"를 사용한 고급 다중 필터 검색(`Alt+F7`), 스포트라이트 및 자연어 의미 체계 명령(`/`), 파일 분할기 및 링커, 체크섬 확인(MD5, SHA-256, CRC32), 보안 멀티패스 삭제(`cm_Wipe`) 및 내장 터미널(`Ctrl+J`).
