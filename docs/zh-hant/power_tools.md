...
┌────────────────────────────────────────────────────────────────────────────────────────┐
│                                 雙欄檔案操作面板                                       │
│        左側面板 (源目錄 / 目錄 A)                   右側面板 (目標目錄 / 目錄 B)       │
├────────────────────────────────────────────────────────────────────────────────────────┤
│                                          │                                             │
│  [1] 多重批次重新命名 (Ctrl+M)             │  [2] 雙欄檔案差異比對 (Meta+Shift+F12)      │
│      動態佔位符、正則反向引用、實時防衝突│      逐行對齊高亮、程式碼塊合併、原地原子儲存 │
│                                          │                                             │
│  [3] 資料夾雙向同步 (Shift+F12)          │  [4] 多功能高階搜尋 (Alt+F7)                │
│      逐位元組雜湊比對、非對稱映象策略      │      Spotlight / 深度遍歷 ➔ 匯出到面板列表  │
│                                          │                                             │
│  [5] 語義化命令輸入欄 (/)                │  [6] 核心安全與實用工具集                   │
│      Spotlight 檢索、自然語言智慧意圖    │      大檔案分割/合併、雜湊計算、多輪安全粉碎│
│                                          │                                             │
├────────────────────────────────────────────────────────────────────────────────────────┤
│  [匯出到面板列表 (Feed to Listbox)] ➔ 將跨目錄搜尋結果生成虛擬標籤頁，統一集中批處理   │
└────────────────────────────────────────────────────────────────────────────────────────┘
```
...
| **開啟系統終端** | `Ctrl+J` / `⌃J` | `Ctrl+J` | `cm_RunTerm` | 在當前面板所在路徑下喚起 macOS 系統終端 |
...
2. 搜尋視窗關閉，當前啟用面板自動開啟一個**全新的虛擬結果標籤頁**。
...
### 7.5 嵌入式系統終端秒開 (`Ctrl+J` / `⌃J` / `cm_RunTerm`)

遇到需要快速執行 Git、執行編譯指令碼或啟動測試服務的場景：

- 按下 **`Ctrl+J`**（`⌃J`）或點選選單欄 **命令 ➔ 執行終端**。
- ATBCmder 會瞬間喚起 macOS 原生 **終端 (Terminal.app)** 或你偏好的 iTerm2。
- **核心便利**：終端新標籤頁的當前工作目錄 (`cwd`) 會**精準初始化為你當前面板開啟的絕對路徑**，徹底免去繁瑣的手敲 `cd /Users/...`。
...
| **實用工具** | 在當前目錄喚起系統終端 | `Ctrl+J` / `⌃J` | `Ctrl+J` | `cm_RunTerm` |
