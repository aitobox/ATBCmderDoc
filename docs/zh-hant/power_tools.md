...
在日常高負荷的檔案管理中，基礎的單檔案複製、移動與刪除僅僅是冰山一角。專業開發者、系統管理員、數字創作者與資料分析師經常面臨一系列複雜的批次與系統級任務：將數千張命名混亂的相機素材批次規範化、快速找出兩版配置補丁之間的細微回退程式碼、在區域網 NAS 陣列之間維護嚴格一致的映像檔備份、深度挖掘埋藏在數十層子目錄裡的配置檔案，以及在歸檔前對海量資產進行加密雜湊核驗。

ATBCmder 將這些原本費時費力的繁瑣勞動轉化為行雲流水的高效流水線。你完全不需要編寫簡陋的 Shell 臨時腳本，也不必在多個第三方小工具或外部比對軟體之間來回切換。ATBCmder 將一整套工業級自動化套件直接植入其正統雙面板核心中。無論是全目錄正規表示式批次替換、基於逐位元組比對的雙向資料夾同步，還是把多條件搜尋結果一鍵裝載至虛擬面板中集中操作，一切盡在純鍵盤極速掌控之中。
...
| **資料夾雙向同步** | `Shift+F12` / `⇧F12` | `Shift+F12` | `cm_SyncDirs` | 深度比對左右面板目錄樹並執行雙向/單向映像檔同步 |
...
| **開啟系統終端機機機** | `Ctrl+J` / `⌃J` | `Ctrl+J` | `cm_RunTerm` | 在當前面板所在路徑下喚起 macOS 系統終端機機機 |
...
*圖 5.2：資料夾同步對話方塊，展示遞迴目錄比對狀態、單雙向流向箭頭以及非對稱映像檔控制開關。*
...
### 4.4 非對稱映像檔 (Asymmetric) vs 雙向對稱同步
...
#### 2. 非對稱映像檔備份 (勾選 `Asymmetric`)
- **核心目標**：使右側備份目錄成為左側源目錄的**絕對完全映像檔複製**。
...
當需要將數十 GB 的系統映像檔、影片素材或虛擬機器檔案透過有限制的介質傳輸（例如 FAT32 的單檔案 4 GB 上限、或郵件附件上限）時，**檔案分割器** (`SplitWorker`) 可以將其切片為連續的碎片包：
...
### 7.5 嵌入式系統終端機機機秒開 (`Ctrl+J` / `⌃J` / `cm_RunTerm`)

遇到需要快速執行 Git、執行編譯腳本或啟動測試服務的場景：

- 按下 **`Ctrl+J`**（`⌃J`）或點選選單欄 **命令 ➔ 執行終端機機機**。
- ATBCmder 會瞬間喚起 macOS 原生 **終端機機機 (Terminal.app)** 或你偏好的 iTerm2。
- **核心便利**：終端機機機新分頁的當前工作目錄 (`cwd`) 會**精準初始化為你當前面板開啟的絕對路徑**，徹底免去繁瑣的手敲 `cd /Users/...`。
...
> [!CAUTION]
> **非對稱目錄映像檔刪除的不可逆風險**  
> 在資料夾同步 (`Shift+F12`) 中勾選 **非對稱映像檔 (Asymmetric)** 模式後，目標路徑中任何源目錄不存在的檔案都會被**物理直接永久刪除**。在點選“同步”之前，務必逐項審閱對比預覽表中的紅色刪除標記。
...
| **實用工具** | 在當前目錄喚起系統終端機機機 | `Ctrl+J` / `⌃J` | `Ctrl+J` | `cm_RunTerm` |
