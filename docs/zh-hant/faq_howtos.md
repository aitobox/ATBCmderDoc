...
│  任務目標                                  推薦工具 / 方法          快捷鍵             │
│  ──────────────────────────────────────    ───────────────────────  ─────────────────  │
│  [1] 將本地專案映象備份到行動硬碟或 NAS    資料夾同步工具          Shift+F12 (⇧F12)    │
│  [2] 按拍攝日期和序號批次規整照片檔案      多重批次重新命名工具      Ctrl+M (⌃M)         │
│  [3] 掛載家庭/辦公 NAS、網盤或雲伺服器     網路連線管理器          cm_ManageConnections│
│  [4] 直接修改 .zip 壓縮包內的配置檔案      壓縮包 VFS + 編輯器     Enter ➔ F4 ➔ 儲存   │
│  [5] 揪出並清理多層深處的大檔案釋放磁碟    扁平展開檢視 / 高階搜尋 Cmd+B (⌘B) / Alt+F7 │
│                                                                                        │
│  常見故障 / 異常現象                       根本原因                解決方案            │
│  ──────────────────────────────────────    ──────────────────────  ──────────────────  │
│  提示 "Operation not permitted" 許可權錯誤   macOS 沙盒 / TCC 攔截   cm_GrantAccess      │
│  外接行動硬碟或 U 盤內容未自動重新整理更新     FAT/exFAT 缺少核心事件 調小輪詢間隔或重新整理   │
│  想大膽測試配置但擔心破壞原有正常設定      生產 XML 隔離保護       執行測試指令碼        │
│  按 F1–F12 變成了調節螢幕亮度和音量        macOS 預設佔用了媒體鍵 配合 Fn 鍵或系統設定 │
│  跨磁碟移動超大檔案耗時明顯變長            跨卷物理“複製+校驗+刪除” 確保目標空間充裕   │
│                                                                                        │
└────────────────────────────────────────────────────────────────────────────────────────┘
```
...
| **扁平分支檢視** | `Cmd+B` / `⌘B` | `Ctrl+B` | `cm_FlatView` | 遞迴穿透所有子資料夾，單列表平鋪顯示全部檔案 |
...
| **開啟系統終端** | `Ctrl+J` / `⌃J` | `Ctrl+J` | `cm_RunTerm` | 在當前面板所在路徑下喚起 macOS 終端視窗 |
...
*圖 9.1：資料夾同步視窗，直觀展示左右雙欄檔案對比、流向箭頭與單向映象備份選項。*
...
5. **選擇同步策略**：
   ...
   - **非對稱映象備份 (勾選 `Asymmetric`)**：以左側為唯一基準，強制右側備份完全一致。**注意：右側存在但左側已刪除的檔案，將被永久刪除！**
6. **確認並執行**：
   - 檢視底部的彙總資料：如 *“從左到右: 42 個檔案 (128.4 MB) | 從右到左: 0 個檔案 | 刪除: 3 個檔案”*。
   - 點選 **同步 (Synchronize)**。系統會在後臺佇列無阻塞傳輸，介面實時顯示速率與剩餘時間。

> [!CAUTION]
> **非對稱映象 (Asymmetric) 刪除風險警告**：
> 勾選 **Asymmetric (非對稱映象)** 後，任何在源目錄中不存在的檔案都會在目標盤中被**徹底刪除**，且不經過廢紙簍！點選“同步”前，請務必核對檔案清單中的紅色刪除項。
...
**場景目標**：將辦公室的群暉/威聯通 NAS、雲端的 Linux 伺服器或私有 Nextcloud 網盤掛載為雙面板的一個標籤頁，像操作本地資料夾一樣自由複製、移動和編輯檔案。
...
6. **一鍵連線瀏覽**：
   - 點選 **`🔗 連線`** 按鈕（或按 `Enter` 鍵）。
   - 當前啟用面板會立刻新建一個標籤頁，路徑以統一虛擬檔案系統 URI 展示：
     ```
...
**場景目標**：迅速找出深層隱藏的廢棄 4K 影片、龐大的 `node_modules` 快取、虛擬機器映象或舊安裝包，釋放寶貴的 Mac 固態硬碟空間。
...
3. **按檔案大小倒序排列**：
   ...
   - 列表中體積最大的巨大 ISO 映象、大型資料庫備份和影片檔案立刻置頂排列在最上方。
4. **統計資料夾佔用空間**：
...
4. **一鍵“匯出到面板列表 (Feed to Listbox)”**：
   ...
   - **魔法發生**：所有符合條件的搜尋結果會被整體裝載進當前面板的一個**專用虛擬標籤頁**中！
   - 與普通的模態彈窗不同，這個虛擬標籤頁與常規檔案面板毫無區別：你可以按 `Cmd+Q` 開啟對側快速預覽逐一審查，也可以按 `F3` 呼叫檢視器開啟。
5. **批次核對與安全刪除**：
...
- 在 ATBCmder 中看到的資料夾空空如也，但在訪達 (Finder) 中卻能看到內容；
...
```
┌─────────────────────────────────────────────────────────────┐
│  授予磁碟讀寫許可權 (Grant Filesystem Access)             [x] │
├─────────────────────────────────────────────────────────────┤
│  為了確保系統安全，本版本 ATBCmder 執行在 macOS 沙盒環境中。│
│  需要您的明確授權才能正常讀寫系統關鍵資料夾與外接驅動器。   │
│                                                             │
│  [  授予根目錄讀寫許可權 (/)  ]                               │
│                                                             │
│  [  授予外接磁碟讀寫許可權 (/Volumes)  ]                      │
...
3. 點選 **“授予外接磁碟讀寫許可權 (/Volumes)”**。
   ...
   * 如此一來，未來插入的所有 USB 快閃記憶體盤、移動固態硬碟、SD 卡、DMG 映象以及 SMB 區域網掛載點均可暢通訪問。
4. 點選 **完成**。授權資訊永久儲存，重啟軟體依然有效。

#### 解決方案 2：在 macOS 系統設定中開啟“完全磁碟訪問許可權”

如果你需要管理系統深度受保護目錄（如 `~/Library/Mail`、`~/Library/Messages`、Safari 瀏覽器快取或時間機器備份等），還需開啟系統級的完全磁碟訪問許可權：

1. 開啟 **系統設定**（點選左上角  蘋果選單 ➔ 系統設定...）。
2. 在左側選單點選 **隱私與安全性 ➔ 完全磁碟訪問許可權**。
...
#### 解決方案 3：透過終端重置損壞的系統 TCC 許可權記錄

如果經歷過 macOS 大版本系統升級後許可權記錄發生混亂，可開啟終端執行如下命令重置 ATBCmder 的隱私許可權：
...
#### 使用專用測試指令碼進行無損試用：`scripts/ATBCmder_test.sh`

如果你想嘗試全新的按鍵佈局、極端配色或自動化指令碼，但又絕不想汙染現有的主力配置檔案，可以使用專案提供的沙盒隔離啟動指令碼：
...
1. 該指令碼會自動在本地建立獨立的臨時目錄：`tests/.test_config/`。
...
├────────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                        │
│   源路徑: /Users/brain/Downloads/BigFile.iso   ➔ 目標路徑: /Users/brain/Movies/        │
│                                                                                        │
│   1. 發起 POSIX rename() 原子系統呼叫，僅僅修改檔案系統目錄樹中的指標記錄。            │
│   2. SSD 固態硬碟上儲存的真實資料塊完全不需要物理讀寫或遷移。                          │
...
├────────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                        │
│   源路徑: /Users/brain/Downloads/BigFile.iso   ➔ 目標路徑: /Volumes/ExternalSSD/Movie/ │
│                                                                                        │
│   階段 1: 二進位制資料流複製（從內建固態硬碟流式讀出 ➔ 寫入外接行動硬碟）                │
│   階段 2: 完整性核對與磁碟落盤刷盤 (fsync 確保外接裝置物理寫入完成)                    │
│   階段 3: 安全移除原始檔（只有當階段 2 驗證無誤後，才解除原始檔佔用）                  │
│                                                                                        │
└────────────────────────────────────────────────────────────────────────────────────────┘
```
...
#### Q2：如何將左右面板的內容瞬間對調？
按下 **`Ctrl+U`**（`⌃U`）或執行命令 **`cm_Exchange`**。左右面板的當前路徑、標籤頁列表與游標位置會瞬間互換。若想將兩欄寬度重置為精確的 50/50，只需雙擊中間的垂直分割線即可。
...
#### Q5：如何快速在當前目錄喚起 macOS 系統終端？
按下 **`Ctrl+J`**（`⌃J`）或執行命令 **`cm_RunTerm`**。ATBCmder 會自動開啟一個新的終端視窗（支援系統 Terminal 或 iTerm2），其初始工作目錄會自動設定為你當前面板所在的絕對路徑。
...
- **保持 UNIX 執行許可權**：在 macOS APFS 磁碟之間複製編譯指令碼或可執行二進位制檔案時，請確保在複製對話方塊（`F5`）中勾選了 **保留檔案屬性與許可權**，系統會自動保留其可執行屬性（`chmod +x`）。
...
<div align="center">
  ...
  <p>
    ...
    <a href="keyboard_shortcuts.md">第 9 章：終極快速鍵全書</a> &nbsp;&bull;&nbsp;
    <a href="download.md">第 11 章：軟體下載與安裝指南 &rarr;</a>
  </p>
</div>
