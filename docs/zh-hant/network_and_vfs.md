...
傳統桌面作業系統往往強迫使用者在割裂的應用之間來回切換：開啟專門的解壓工具將 ZIP 解包到臨時目錄，改完後再重新打包；開啟第三方 FTP/SFTP 客戶端上傳檔案；或者透過作業系統掛載彈窗把遠端網路盤散亂堆放在互不相通的Finder (Finder) 視窗中。
...
- **多標籤虛擬工作區**：在左側面板分頁 1 開啟遠端 SFTP 伺服器，分頁 2 開啟本地加密 ZIP 包，分頁 3 開啟本地 `~/Downloads` 目錄，多源自由切換。
- **定向雙向複製 (`F5`)**：在本地啟用面板中選中檔案並按下 `F5`，即可將其直接推送到對側面板展示的遠端伺服器或壓縮包內部。
- **直接拖曳傳輸 (Drag-and-Drop)**：跨越面板邊界，在本地磁碟、網路共享以及歸檔內部層級之間直接拖曳檔案，無需中間繁瑣落地。
...
- **現存掛載點智慧感知**：若 macOS Finder或系統開機腳本已經掛載了目標 SMB 卷，ATBCmder 會自動從系統的 `mount` 掛載表中嗅探並瞬間定位進入，避免重複建立冗餘連線。
...
4. 點選 **連線**。ATBCmder 會立即在當前面板新建一個分頁，直連遠端伺服器。
...
- **立即連線 (`🔗 立即連線`)**：儲存改動，立即建立遠端會話並在新分頁中開啟。
...
在各類針對開發者的網路安全攻擊中，檔案管理器的配置檔案往往是重災區。許多傳統檔案管理工具把 FTP/SFTP 密碼明文存放在使用者家目錄的 XML 或 INI 文字中，極易被惡意掃描腳本盜取。
...
當跨越遠端伺服器複製海量目錄，或透過 SFTP 下載數十 GB 的系統映像檔時，切勿讓介面凍結。ATBCmder 的所有網路與歸檔操作均原生接入 **後臺任務佇列系統**：
...
### 技巧 2：異構協議間的點對點流式直傳
ATBCmder 內建的 `stream_copy_file` 引擎支援**跨協議直接管道流式傳輸**。例如將左側面板 SFTP 伺服器上的大資料夾直接拖曳到右側面板的內網 SMB 共享盤中：
...
需要直接修改遠端 Linux 伺服器上的 `nginx.conf` 或 Python 腳本？
